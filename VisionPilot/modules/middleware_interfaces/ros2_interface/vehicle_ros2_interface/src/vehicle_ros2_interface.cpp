    ...
    : rclcpp::Node("vehicle_ros2_interface")
{
    ...
    speed_sub_ = create_subscription<std_msgs::msg::Float32>(
        "/vehicle/speed", qos,
        [on_speed](const std_msgs::msg::Float32::SharedPtr msg) {
            on_speed(static_cast<double>(msg->data));
        });

    // Reliable depth-1 QoS for commands — must not be silently dropped.
    auto cmd_qos = rclcpp::QoS(rclcpp::KeepLast(1)).reliable();

    steering_pub_ = create_publisher<std_msgs::msg::Float32>("/vehicle/steering_cmd", cmd_qos);
    throttle_pub_ = create_publisher<std_msgs::msg::Float32>("/vehicle/throttle_cmd", cmd_qos);
...
void VehicleRos2Interface::write(const double steering, const double acceleration)
{
    ...
    throttle_msg.data = static_cast<float>(acceleration);
