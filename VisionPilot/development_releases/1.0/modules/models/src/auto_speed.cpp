...
// ─────────────────────────────────────────────────────────────────────────────
// Internal NMS helpers (file-scope, not exposed in the header)
// ─────────────────────────────────────────────────────────────────────────────

float AutoSpeedOnnx::iou(const Detection& a, const Detection& b)
{
...
std::vector<Detection> AutoSpeedOnnx::nms(
...
AutoSpeedOnnx::AutoSpeedOnnx(const OnnxSessionConfig& cfg)
    : env_(ORT_LOGGING_LEVEL_WARNING, "AutoSpeed")
    ...
    , input_shape_{1, 3, NET_H, NET_W}
{
    ...
    for (size_t i = 0; i < n_in; ++i) {
        ...
        printf("[AutoSpeedOnnx] input[%zu]  = %s\n", i, in_names_[i]);
    }
    ...
    for (size_t i = 0; i < n_out; ++i) {
        ...
        printf("[AutoSpeedOnnx] output[%zu] = %s\n", i, out_names_[i]);
    }

    printf("[AutoSpeedOnnx] Initialized — %zu inputs, %zu outputs | "
           "frame shape [1, 3, %d, %d]\n",
...
AutoSpeedOutput AutoSpeedOnnx::infer(
    const float* image_chw, float conf_thres, float iou_thres)
{
    ...
    try {
        ...
        printf("[AutoSpeedOnnx] Inference error: %s\n", e.what());
        return out;
    }

    if (results.empty()) {
        printf("[AutoSpeedOnnx] No output tensors returned\n");
        return out;
    }

    return postProcess(results[0], conf_thres, iou_thres);
}

// ─── Post-processing ─────────────────────────────────────────────────────────

AutoSpeedOutput AutoSpeedOnnx::postProcess(
    const Ort::Value& tensor, float conf_thres, float iou_thres) const
{
    AutoSpeedOutput out;

    const auto shape = tensor.GetTensorTypeAndShapeInfo().GetShape();
    // Expected: [1, C, N]  where  C = 4 + num_classes
    if (shape.size() < 3) {
        printf("[AutoSpeedOnnx] Unexpected output rank: %zu\n", shape.size());
        return out;
    }
    ...
    if (num_classes <= 0) {
        printf("[AutoSpeedOnnx] Invalid channel count: C=%lld\n",
               static_cast<long long>(C));
        return out;
    }

    // Memory layout: data[c * N + n]  (channel-first, batch squeezed)
